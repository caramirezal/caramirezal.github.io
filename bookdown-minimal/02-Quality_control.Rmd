...
png('figures/qc_ncount_vs_features.png')
FeatureScatter(pbmc.filtered, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
dev.off()
```
...
```{r, qc_metrics_exercise_1, include=FALSE}
pbmc.seurat[["percent.mt"]] <- PercentageFeatureSet(pbmc.seurat, pattern = "^MT-")
summary(pbmc.seurat$percent.mt)
```
...
```{r, example_matrix,include=FALSE}
examp.matrix <- read.table('https://raw.githubusercontent.com/caramirezal/caramirezal.github.io/master/bookdown-minimal/data/pbmc_10X_250_cells.tsv',
                           sep = '\t')

examp.matrix %>% dim
examp.matrix[1:5, 1:5]

example.seurat <- CreateSeuratObject(
  counts = examp.matrix, 
  project = 'PBMC', 
  assay = 'RNA', 
  min.cells = 1, 
...
example.seurat[["percent.mt"]] <- PercentageFeatureSet(example.seurat, 
                                                       pattern = "^MT-")
example.seurat@meta.data %>% head

VlnPlot(example.seurat, features = c('nCount_RNA', 
                                     'nFeature_RNA',
                                     'percent.mt'))

pbmc.filtered <- subset(example.seurat, 
                           nFeature_RNA < 1250 &
                                nCount_RNA < 4000 & 
                                    percent.mt < 5)
```
